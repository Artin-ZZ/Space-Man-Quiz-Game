...
class MainWindowM(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.c.execute("""CREATE TABLE IF NOT EXISTS Math(
        ...
        self.setWindowTitle("Math Managment")
...
class DeleteDialog(QDialog):
    ...
    def deletestudent(self):
        ...
        try:
            self.conn = sqlite3.connect("questions.db")
            self.c = self.conn.cursor()
            self.c.execute("DELETE from Math WHERE id="+str(delrol))
            self.conn.commit()
            self.c.close()
            self.conn.close()
    ...
    def add_qu(self):
        ...
        while True:
            ...
            try:
                self.conn = sqlite3.connect("questions.db")
                self.c = self.conn.cursor()
                self.c.execute("INSERT INTO Math (question,one,two,three,four,answer,id) VALUES (?,?,?,?,?,?,?)",(qu_inp,one,two,three,four,answer,ra_id))
                self.conn.commit()
                self.c.close()
                self.conn.close()
