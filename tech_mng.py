...
#  Mainwindow Class    #
########################
class MainWindowT(QMainWindow):
    # Constractor Method
    def __init__(self, *args, **kwargs):
        super(MainWindowT, self).__init__(*args, **kwargs)

        self.conn = sqlite3.connect('questions.db')
        self.c = self.conn.cursor()
        self.c.execute("""CREATE TABLE IF NOT EXISTS Tech(
        ...
        self.setWindowTitle("Technology Managment")
...
class DeleteDialog(QDialog):
    ...
    def deletestudent(self):
        ...
        try:
            self.conn = sqlite3.connect("questions.db")
            self.c = self.conn.cursor()
            self.c.execute("DELETE from Tech WHERE id="+str(delrol))
            self.conn.commit()
            self.c.close()
            self.conn.close()
    ...
    def add_qu(self):
        ...
        while True:
            ...
            try:
                self.conn = sqlite3.connect("questions.db")
                self.c = self.conn.cursor()
                self.c.execute("INSERT INTO Tech (question,one,two,three,four,answer,id) VALUES (?,?,?,?,?,?,?)",(qu_inp,one,two,three,four,answer,ra_id))
                self.conn.commit()
                self.c.close()
                self.conn.close()
